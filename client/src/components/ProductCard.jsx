    ...
    >
      <Link to={`/product/${product.id}`} className="group block">
        <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-ink">
          ...
          />
          {product.tag && (
            <span className="absolute top-3 left-3 bg-parchment text-ink text-[10px] font-condensed tracking-[0.12em] px-2.5 py-1 rounded-full">
        ...
        <div className="pt-4 px-0.5">
          {(product.verse || product.tagline) && (
            <p className="text-[11px] font-condensed tracking-[0.1em] text-gold mb-1">
              {product.verse || product.tagline}
            </p>
          )}
